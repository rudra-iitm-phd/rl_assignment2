...
def custom_trainer(env, agent, n_episodes=10000, max_t=1000, eps_start=1, eps_end=0.01, eps_decay=0.985):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            action = agent.act(state, eps)
            next_state, next_reward, done, _ , _ = env.step(action)
            agent.step(reward, state, action, next_reward, next_state, done)
            state = next_state
            reward = next_reward
            score += reward
            if done:
